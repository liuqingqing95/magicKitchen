...
import { useKeyboardControls, useGLTF, useTexture, useAnimations } from '@react-three/drei'
...
export default function Player()
{
    ...
    const [capsuleSize, setCapsuleSize] = useState([0.5, 1])
    ...
    const debugWireMat = useMemo(() => new THREE.MeshBasicMaterial({ color: 'cyan', wireframe: true, transparent: true, opacity: 0.6 }), [])
  ...
  useEffect(() => {
    ...
    if (!any) {
      ...
      console.log('computed model world bbox:', totalBox.min, totalBox.max, 'modelBottomOffset:', modelBottomOffset.current);
    }
  }, [hamburger]);
  ...
  useEffect(() => {
    ...
    useFrame((state, delta) => {
        ...
        if (prevSprinting.current && !isSprinting && inputMoving && body.current) {
          ...
          // 应用一次性冲量
          body.current.applyImpulse({ x: forwardDir.x * SPRINT_GLIDE_IMPULSE, y: 0, z: forwardDir.z * SPRINT_GLIDE_IMPULSE })
        }
        ...
        if (target.lengthSq() > 1e-6) target.normalize().multiplyScalar(maxSpeed);
        ...
        if (actions) {
            ...
            if (inputMoving) {
                if (isSprinting && sprint) {
                    ...
                    const prevSprint = sprint.getEffectiveWeight ? sprint.getEffectiveWeight() : (sprint._effectiveWeight || 0);
                    const newSprintW = THREE.MathUtils.lerp(prevSprint, sprintWeight, Math.min(1, 10 * delta));
                    sprint.setEffectiveWeight(newSprintW);
                    sprint._effectiveWeight = newSprintW;

                    // 减少 walk 动画权重
                    if (walk) {
                        const adjustedWalkWeight = Math.max(0, walk._effectiveWeight - newSprintW);
                    ...
                    // 播放 walk 动画
                    const walkWeight = 1;
                    const prevWalk = walk.getEffectiveWeight ? walk.getEffectiveWeight() : (walk._effectiveWeight || 0);
                    const newWalkW = THREE.MathUtils.lerp(prevWalk, walkWeight, Math.min(1, 10 * delta));
                    walk.setEffectiveWeight(newWalkW);
                    walk._effectiveWeight = newWalkW;

                    // 减少 sprint 动画权重
                    if (sprint) {
                        const adjustedSprintWeight = Math.max(0, sprint._effectiveWeight - newWalkW);
        ...
        if(modelRef.current && body.current){
            ...
            const visualY = p.y + COLLIDER_OFFSET_Y - capsuleHalf - (modelBottomOffset.current || 0) + VISUAL_ADJUST
            ...
            if (inputMoving) {
              ...
              hasTarget = true
            } else if (speed > 0.25) { // 小阈值避免被轻微碰撞扰动
              horiz.normalize()
            ...
            if (actions) {
                ...
                const walk = actions['Walk'] || actions['walk'] || Object.values(actions)[0]; // 获取 walk 动画
            
                if (walk) {
                    const inputSpeed = Math.min(1, (inputMoving ? Math.sqrt(inputX * inputX + inputZ * inputZ) : 0) * (maxSpeed > 0 ? 1 : 0));
                    const targetWeight = inputMoving ? inputSpeed : 0;
                    const prev = walk.getEffectiveWeight ? walk.getEffectiveWeight() : (walk._effectiveWeight || 0);
                    const newW = THREE.MathUtils.lerp(prev, targetWeight, Math.min(1, 10 * delta));
    ...
    <primitive ref={ modelRef } rotation={[0, Math.PI, 0]} object={ hamburger.scene } scale={ 1 }  />
    </>
}